...
const RepoDetail: React.FC = () => {
  ...
  if (!repo || !owner || !name) {
    return (
      <div className="p-6">
        <p style={{ color: 'var(--text-tertiary)' }}>Repository not found.</p>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          {repo.description && (
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{repo.description}</p>
          )}
        </div>
      </div>
...
const CodeTab: React.FC<{ owner: string; name: string }> = ({ owner, name }) => {
  ...
  useEffect(() => {
    ...
    fetchRepoContents(owner, name, currentPath)
      ...
      .catch(() => {
        // Might be a file, try to fetch as file
        fetchFileContent(owner, name, currentPath)
  ...
  const navigateTo = (item: GitHubContent) => {
    if (item.type === 'dir') {
  ...
  if (fileContent !== null) {
    return (
      <div>
        <Breadcrumbs parts={breadcrumbs} onNavigate={(path) => setCurrentPath(path)} />
        <div className="border rounded-lg overflow-hidden mt-2" style={{ borderColor: 'var(--border)' }}>
          <div className="px-4 py-2 text-xs border-b" style={{ borderColor: 'var(--border)', background: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>
            {breadcrumbs[breadcrumbs.length - 1]}
          </div>
          <pre className="p-4 text-sm overflow-auto max-h-[60vh] font-mono leading-relaxed"
  ...
  return (
    <div>
      {currentPath && <Breadcrumbs parts={breadcrumbs} onNavigate={(path) => setCurrentPath(path)} />}
      <div className="border rounded-lg divide-y mt-2" style={{ borderColor: 'var(--border)', background: 'var(--bg-secondary)' }}>
        {currentPath && (
          <div className="px-4 py-2 cursor-pointer hover:bg-[var(--bg-tertiary)] transition-colors flex items-center gap-2 text-sm"
               onClick={goUp} style={{ color: 'var(--text-secondary)' }}>
            ..
          </div>
        )}
        {contents.map(item => (
        ...
        {contents.length === 0 && (
          <p className="text-center py-6 text-sm" style={{ color: 'var(--text-tertiary)' }}>Empty directory</p>
        )}
      </div>
    </div>
  );
...
const Breadcrumbs: React.FC<{ parts: string[]; onNavigate: (path: string) => void }> = ({ parts, onNavigate }) => (
  <div className="flex items-center gap-1 text-sm flex-wrap">
    <button onClick={() => onNavigate('')} className="font-medium" style={{ color: 'var(--accent)' }}>
      root
    </button>
    {parts.map((part, i) => (
      <React.Fragment key={i}>
        ...
        <button
          ...
          style={{ color: i === parts.length - 1 ? 'var(--text-primary)' : 'var(--accent)' }}
        >
          {part}
        </button>
      </React.Fragment>
...
const IssuesTab: React.FC<{ owner: string; name: string }> = ({ owner, name }) => {
  ...
  if (loading) {
    return <div className="flex justify-center py-12"><Loader2 className="w-5 h-5 animate-spin" style={{ color: 'var(--text-tertiary)' }} /></div>;
  }

  if (issues.length === 0) {
    return <p className="text-center py-8 text-sm" style={{ color: 'var(--text-tertiary)' }}>No issues.</p>;
  }
...
const PRsTab: React.FC<{ owner: string; name: string }> = ({ owner, name }) => {
  ...
  if (prs.length === 0) {
    return <p className="text-center py-8 text-sm" style={{ color: 'var(--text-tertiary)' }}>No pull requests.</p>;
  }
...
const CommitsTab: React.FC<{ owner: string; name: string }> = ({ owner, name }) => {
  ...
  if (commits.length === 0) {
    return <p className="text-center py-8 text-sm" style={{ color: 'var(--text-tertiary)' }}>No commits.</p>;
  }
        ...
        <div key={c.sha} className="px-4 py-3 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1 text-xs" style={{ color: 'var(--text-tertiary)' }}>
              {c.author && (
                <span className="flex items-center gap-1">
                  <img src={c.author.avatar_url} alt="" className="w-3.5 h-3.5 rounded-full" />
                  {c.author.login}
                </span>
              )}
              {!c.author && <span>{c.commit.author.name}</span>}
