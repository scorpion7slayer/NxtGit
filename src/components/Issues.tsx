...
import { fetchUserIssues, repoNameFromUrl, parseRepoFromUrl, timeAgo, type GitHubIssue } from '../lib/github';
    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                 placeholder="Filter issues..." className="input-glass pl-9 w-full text-sm py-2" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="border rounded-lg divide-y" style={{ borderColor: 'var(--border)', background: 'var(--bg-secondary)' }}>
          {filtered.map(issue => {
            const repo = parseRepoFromUrl(issue.repository_url);
            return (
              <div key={issue.id}
                   className="px-4 py-3 cursor-pointer hover:bg-[var(--bg-tertiary)] transition-colors"
                   onClick={() => navigate(`/issue/${repo.owner}/${repo.name}/${issue.number}`)}>
                <div className="flex items-start gap-2">
                  ...
                  {issue.comments > 0 && (
                    <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{issue.comments}</span>
                  )}
                </div>
              </div>
            );
