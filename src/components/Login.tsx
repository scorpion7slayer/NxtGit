...
const Login: React.FC = () => {
        ...
        try {
            ...
            try {
                ...
                pollingRef.current = setTimeout(
                    () => pollForToken(deviceCode, interval, expiresAt),
                    interval,
                );
                return;
            }
            ...
            if (
                ...
                pollingRef.current = setTimeout(
                    () => pollForToken(deviceCode, nextInterval, expiresAt),
                    nextInterval,
                );
                return;
            }
    ...
    const handleGitHubLogin = async () => {
        if (!CLIENT_ID) {
            setError(
                "GitHub Client ID not configured. Add VITE_GITHUB_CLIENT_ID to .env",
            );
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setError(
                    `GitHub error ${response.status}: ${body}. Make sure Device Flow is enabled.`,
                );
                setIsLoading(false);
                return;
            }
            ...
            // Validate URI to prevent open redirect
            if (
                !data.verification_uri ||
                !data.verification_uri.startsWith("https://github.com/")
            ) {
                setError("Invalid verification URI from GitHub");
                setIsLoading(false);
                return;
            }
            ...
            <div className="max-w-sm w-full login-card">
                <div className="text-center mb-8">
                    <img
                        src={logo}
                        alt="NxtGit"
                        className="w-16 h-16 mx-auto mb-5 rounded-xl"
                    />
                    <h1
                ...
                {error && (
                    <div
                        className="flex items-start gap-2 px-3 py-2.5 mb-4 rounded-lg text-sm"
                        style={{
